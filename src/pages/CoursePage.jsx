        ...
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: 'var(--gray-600)' }}>
              {hasKey
                ? `키: ${API_KEY.slice(0, 8)}${'*'.repeat(Math.max(0, API_KEY.length - 8))}`
      ...
      <div className="section">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <button
            ...
            onClick={() => runTest('연결 테스트', testApiConnection)}
          >
            {loading && activeTest === '연결 테스트' ? <span className="spinner" /> : null}
            1. 연결 테스트 (광주 관광지 1건)
          </button>
          <button
            className="btn btn-outline"
            disabled={!hasKey || loading}
            onClick={() => runTest('광주 관광지 목록', () => getAreaBasedList({ areaCode: AREA_CODES.GWANGJU, numOfRows: 5 }))}
          >
            2. 광주 관광지 목록 (5건)
          </button>
          <button
            className="btn btn-outline"
            disabled={!hasKey || loading}
            onClick={() => runTest('전남 관광지 목록', () => getAreaBasedList({ areaCode: AREA_CODES.JEONNAM, numOfRows: 5 }))}
          >
            3. 전남 관광지 목록 (5건)
          </button>
          <button
            className="btn btn-outline"
            disabled={!hasKey || loading}
            onClick={() => runTest('여행코스 조회', () => getTourCourseList({ areaCode: AREA_CODES.GWANGJU }))}
          >
            4. 광주 여행코스 조회
          </button>
        </div>
